...
class InternshalaScraper:
    ...
    def _fetch_internship_details(self, detail_url: str) -> Tuple[Optional[str], Optional[str], List[str]]:
        try:
            ...
            description_meta = soup.find('meta', attrs={'name': 'twitter:description', 'property': 'og:description', 'itemprop': 'description'})
    ...
    def _extract_company_info(self, company_profile_url: str) -> Optional[Company]:
        try:
            ...
            company_name_elem = company_info_container.find('div', class_='heading-text') if company_info_container else None
    ...
    def _process_listing(self, listing: BeautifulSoup, companies: Dict[str, Company]) -> Tuple[Optional[Internship], List[str]]:
        try:
            ...
            duration_elem = detail_elem.find_all('div', class_='row-1-item')[1] if detail_elem and len(detail_elem.find_all('div', class_='row-1-item')) > 1 else None
    ...
    def _process_page(self, html_content: str, companies: Dict[str, Company]) -> Dict[Internship, List[str]]:
        ...
        for listing in listings:
            ...
            if internship is not None:
                ...
                self.logger.warning(f"Skipped processing a listing due to an error or missing data")
